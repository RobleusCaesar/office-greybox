...
func _build_sfx() -> void:
	...
	if FileAccess.file_exists("res://audio/shotgun_cocking.wav"):
		_cock_sfx.stream = load("res://audio/shotgun_cocking.wav")
	add_child(_cock_sfx)
	_reload_sfx = AudioStreamPlayer.new()
	_reload_sfx.name = "ReloadSfx"
	_reload_sfx.volume_db = -3.0
	if FileAccess.file_exists("res://audio/shotgun_reloading.wav"):
		_reload_sfx.stream = load("res://audio/shotgun_reloading.wav")
