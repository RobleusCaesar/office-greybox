extends SceneTree
## Headless QA: title is main scene, no east-hall LOS to the window, demons die.


func _init() -> void:
	call_deferred("_run")


func _run() -> void:
	var errors: PackedStringArray = PackedStringArray()
	var main_scene := str(ProjectSettings.get_setting("application/run/main_scene"))
	if main_scene != "res://scenes/title.tscn":
		errors.append("main_scene is %s, expected title.tscn" % main_scene)
	var renderer := str(ProjectSettings.get_setting("rendering/renderer/rendering_method"))
	if renderer != "gl_compatibility":
		errors.append("renderer is %s, expected gl_compatibility" % renderer)

	var packed: PackedScene = load("res://scenes/level.tscn")
	if packed == null:
		errors.append("could not load level.tscn")
		_finish(errors)
		return
	var level: Node = packed.instantiate()
	root.add_child(level)
	for _i in 6:
		await process_frame

	var space: PhysicsDirectSpaceState3D = level.get_world_3d().direct_space_state
	var window := Vector3(38.1, 1.7, 11.5)
	var samples := [
		Vector3(8.5, 1.7, 12.0),
		Vector3(14.5, 1.7, 12.0),
		Vector3(17.4, 1.7, 12.0),
		Vector3(3.5, 1.7, 8.8),
		Vector3(3.5, 1.7, 2.1),
	]
	for origin in samples:
		var q := PhysicsRayQueryParameters3D.create(origin, window)
		q.collision_mask = 1
		var hit := space.intersect_ray(q)
		if hit.is_empty():
			errors.append("LOS clear from %s to window" % origin)
			continue
		var hx: float = hit.position.x
		if hx > 30.0:
			errors.append("Ray from %s reached past reception (hit x=%.2f)" % [origin, hx])

	var d1 := level.get_node_or_null("Demon_01")
	var d2 := level.get_node_or_null("Demon_02")
	if d1 == null or d2 == null:
		errors.append("demons missing after level ready")
	else:
		d1.take_damage(200.0)
		d2.take_damage(200.0)
		await process_frame
		if is_instance_valid(d1) and d1.hp > 0.0:
			errors.append("demon 1 survived 200 damage")
		if is_instance_valid(d2) and d2.hp > 0.0:
			errors.append("demon 2 survived 200 damage")

	var player := level.get_node_or_null("Player")
	if player == null or not player.has_method("take_damage"):
		errors.append("player missing take_damage")
	else:
		player.take_damage(200.0)
		if not player.dead:
			errors.append("player did not die from 200 damage")

	_finish(errors)


func _finish(errors: PackedStringArray) -> void:
	if errors.is_empty():
		print("QA_OK title, renderer, LOS blocked, demons killable, player can die")
		quit(0)
	else:
		for e in errors:
			push_error("QA_FAIL " + e)
			print("QA_FAIL ", e)
		quit(1)
